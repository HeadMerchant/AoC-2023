...
def map_next(in_seeds, in_ranges):
    ...
    for dst, src, length in yield_until_blank():
        ...
        for start, end in list(in_ranges):
            if start <= src < end:
                next_ranges.add((src+convert, min(end, src_end)+convert))
                # in_ranges.add((start, src))
            elif src <= start < src_end:
                next_ranges.add((start+convert, min(end, src_end)+convert))
                # in_ranges.add((src_end, end))
            else:
                continue
            # in_ranges.remove((start, end))
